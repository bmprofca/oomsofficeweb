...
export default function DscDetailsModal({ open, dsc, onClose, onEdit }) {
        ...
        >
          ...
          >
            <div className="shrink-0 flex items-center justify-between gap-3 px-5 py-3.5 border-b border-gray-200">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <h3 className="text-base font-bold text-gray-800 m-0">DSC details</h3>
                  <p className="text-xs text-gray-500 m-0 truncate">{dsc.dsc_id}</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className={MODAL_BODY}>
              <DetailRow label="Company">{dsc.companyName || dsc.company || "—"}</DetailRow>
              <DetailRow label="Type">
                <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">
                  {dsc.typeName || dsc.type || "—"}
                </span>
              </DetailRow>
              <DetailRow label="Issue date">{formatDate(dsc.validity_start)}</DetailRow>
              <DetailRow label="Expire date">{formatDate(dsc.validity_end)}</DetailRow>
              <DetailRow label="Duration">{dsc.duration ? `${dsc.duration} year${Number(dsc.duration) === 1 ? "" : "s"}` : "—"}</DetailRow>
              <DetailRow label="Password">
                {dsc.password ? <span className="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{dsc.password}</span> : "—"}
              </DetailRow>
              ...
              <DetailRow label="Client">{dsc.name || "—"}</DetailRow>
              {dsc.guardian_name ? <DetailRow label="Guardian">{dsc.guardian_name}</DetailRow> : null}
              <DetailRow label="Mobile">{dsc.mobile || "—"}</DetailRow>
              <DetailRow label="Email" last>
                {dsc.email || "—"}
              </DetailRow>
            </div>
