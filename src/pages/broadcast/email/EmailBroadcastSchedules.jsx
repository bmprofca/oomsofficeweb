...
const EmailBroadcastSchedules = () => {
  ...
  const fetchSchedules = useCallback(async () => {
    setLoading(true);
    try {
      const res = await emailApi.listBroadcastSchedules();
      // ok() may return { success, data } with data as the array, or data may already be unwrapped/array
  ...
  const toggleActive = async (row) => {
    ...
    try {
      ...
      await emailApi.updateBroadcastSchedule(row.schedule_id, {
        is_active: next,
      });
  ...
  const runNow = async (row) => {
    ...
    setBusyId(row.schedule_id);
    try {
      const res = await emailApi.runBroadcastSchedule(row.schedule_id);
      const broadcastId =
        res?.data?.broadcast_id || res?.broadcast_id;
      toast.success(res?.message || "Broadcast created from schedule");
      if (broadcastId) {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to run schedule",
      );
    } finally {
      setBusyId(null);
    }
  ...
  const runDelete = async () => {
    ...
    try {
      await emailApi.deleteBroadcastSchedule(confirmRow.schedule_id);
              ...
              <div className="flex items-center gap-2 shrink-0 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-500 m-0 truncate">
                    Cron-style email broadcasts (IST)
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 ml-auto min-w-0 w-full sm:w-auto flex-wrap sm:flex-nowrap">
                ...
                {canSend ? (
                  <button
                    ...
                    className={`${TOOLBAR_BTN} inline-flex items-center gap-1.5 text-white bg-indigo-600 hover:bg-indigo-700 shrink-0`}
                  >
                      ...
                      {rows.map((row, index) => {
                        ...
                        return (
                          <tr key={row.schedule_id} className={TABLE_ROW}>
                            <td
                              className={`${TABLE_TD} text-[11px] font-bold text-gray-800`}
                            >
                              {index + 1}
                            </td>
                            ...
                            <td className={TABLE_TD}>
                              ...
                              {row.last_error ? (
                                <p
                                  className="text-[11px] text-red-600 m-0 truncate"
                                  title={row.last_error}
                                >
                                  {row.last_error}
                                </p>
                              ) : row.last_broadcast_id ? (
                                <button
                                  type="button"
                                  className="text-[11px] text-indigo-700 hover:underline m-0"
                                  onClick={() =>
                                    navigate(
                                      `/broadcast/email/details/${row.last_broadcast_id}`,
                                    )
                                  }
                                >
                                  View last campaign
                        ...
                        {canSend ? (
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              disabled={busy}
                              onClick={() => runNow(row)}
                              className="flex-1 inline-flex items-center justify-center gap-1 px-2 py-1.5 text-xs font-medium rounded-lg border border-indigo-200 text-indigo-700 bg-indigo-50"
                            >
...
export default EmailBroadcastSchedules;
