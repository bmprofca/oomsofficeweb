...
import { AddExpenseModal } from '../staff/ExpenseTab';
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className={`rounded-lg border border-slate-200 px-3 py-2 ${sk} h-14`} />
        ))}
      </div>
      <div className="overflow-hidden rounded-lg border border-slate-200/80 bg-white/70" aria-busy="true">
        <div className="border-b border-slate-200 bg-gradient-to-r from-slate-100/90 via-white to-emerald-50/40 py-2.5 px-3 sm:px-4 space-y-2.5">
          <div className="flex flex-col gap-2 xl:flex-row xl:items-center">
            ...
            <div className={`${sk} h-9 w-9 rounded-lg shrink-0`} />
          </div>
        </div>
...
export default function StaffExpensesReview() {
  ...
  }, [canView, page, limit, debouncedSearch, statusFilter, fromDate, toDate]);
  ...
  const handleCreateExpense = async (newExpenseData) => {
    if (!canFinanceEntry) {
      ...
      throw new Error('Need Access Permission');
    }
    if (!newExpenseData.staff_username) {
    ...
    const payload = {
      ...
      staff_username: newExpenseData.staff_username,
    };
    ...
    () => [
      {
        ...
        amountIdle: 'text-slate-800',
        amountActive: 'text-white',
      },
      {
        key: 'pending',
        value: '0',
        ...
        idle: 'border-amber-200 bg-amber-50/70 text-amber-700 hover:border-amber-300 hover:bg-amber-50',
        active: 'border-amber-600 bg-amber-500 text-white shadow-sm',
        iconIdle: 'text-amber-500',
        iconActive: 'text-amber-100',
        amountIdle: 'text-amber-900',
        amountActive: 'text-white',
      },
      {
        key: 'approved',
        ...
        idle: 'border-emerald-200 bg-emerald-50/70 text-emerald-700 hover:border-emerald-300 hover:bg-emerald-50',
        active: 'border-emerald-600 bg-emerald-600 text-white shadow-sm',
        iconIdle: 'text-emerald-500',
        iconActive: 'text-emerald-100',
        amountIdle: 'text-emerald-900',
        amountActive: 'text-white',
      },
      {
        key: 'cancelled',
        ...
        idle: 'border-rose-200 bg-rose-50/70 text-rose-700 hover:border-rose-300 hover:bg-rose-50',
        active: 'border-rose-600 bg-rose-500 text-white shadow-sm',
        iconIdle: 'text-rose-500',
        iconActive: 'text-rose-100',
        amountIdle: 'text-rose-900',
        amountActive: 'text-white',
      },
    ],
    [summary]
  ...
  return (
    <>
      ...
      <div className={`pt-16 transition-all duration-300 ease-in-out ${isMinimized ? 'md:pl-20' : 'md:pl-[260px]'}`}>
        <div className="h-full flex flex-col mx-2 sm:mx-4 md:mx-8 my-3 md:my-4 space-y-3">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Icon
                          className={`h-3.5 w-3.5 shrink-0 ${isActive ? card.iconActive : card.iconIdle}`}
                          aria-hidden
                        />
                      </div>
                      <p
                        className={`mt-1 text-sm font-bold tabular-nums ${
                          isActive ? card.amountActive : card.amountIdle
                        }`}
                      >
                        {card.count}{' '}
                        <span className={`text-[11px] font-semibold ${isActive ? 'opacity-90' : 'opacity-80'}`}>
              ...
              >
                <div className="sticky top-0 z-10 border-b border-slate-200 bg-gradient-to-r from-slate-100/90 via-white to-emerald-50/40 py-2.5 pl-3 pr-3 sm:pl-4 sm:pr-3 space-y-2.5">
                  <div className="flex flex-col gap-2 min-w-0 xl:flex-row xl:items-center xl:gap-3">
                    ...
                    <div className="relative w-full min-w-0 flex-1 xl:min-w-[10rem]">
                    ...
                    <div className="w-full min-w-0 sm:w-56 xl:w-64 shrink-0">
                      <DateRangePickerField
                        ...
                        showRangeHint={false}
                        showResetButton
                        buttonClassName="h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm justify-between text-left"
                    ...
                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                      <button
                        type="button"
                        onClick={loadList}
                        className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 sm:w-auto sm:gap-1.5 sm:px-3 sm:text-xs sm:font-semibold"
                        title="Refresh"
                        aria-label="Refresh"
                      >
                      ...
                      {canFinanceEntry ? (
                        <button
                          ...
                          className="inline-flex h-9 shrink-0 items-center justify-center gap-1.5 rounded-lg bg-teal-600 px-3 text-xs font-semibold text-white hover:bg-teal-700"
                        >
                          ...
                          rows.map((row, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-3 py-2.5">
                                  <span
                                    className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${statusBadge(
                                      row.status_text
                                    )}`}
                                  >
