...
const getPaymentFromBadgeStyle = (accountType) => {
...
const getPaymentFromInfo = (item) => {
    ...
    if (partyType === 'capital') {
        return {
            ...
            subtitle: '',
            badgeLabel: 'capital',
            ...getPaymentFromBadgeStyle('capital'),
        };
    }
    ...
    if (partyType === 'bank') {
        ...
        return {
            ...
            ...getPaymentFromBadgeStyle(isCash ? 'cash' : accountType),
        };
    }
                    ...
                    >
                        <div className="flex shrink-0 items-center justify-between gap-2 border-b border-blue-500/25 bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-2.5 text-white">
                            <div className="flex min-w-0 items-center gap-2">
                                ...
                                <h2 className="truncate text-sm font-semibold">{record.invoice_no || 'Payment entry'}</h2>
                            </div>
                            <button
                                type="button"
                                onClick={onClose}
                        ...
                        <div className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain px-5 py-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                            {(() => {
                                const partyInfo = getPaymentToInfo(record);
                                const fromInfo = getPaymentFromInfo(record);
                                ...
                                return (
                                    <>
                                        ...
                                        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                                            <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Paid from</p>
                                            <DetailRow label="Account">{fromInfo.displayName}</DetailRow>
                                            {fromInfo.subtitle ? (
                                                <DetailRow label="Account no.">{fromInfo.subtitle}</DetailRow>
                                            ) : null}
                                            <DetailRow label="Type">
                                                <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium capitalize ${fromInfo.bgColor} ${fromInfo.textColor}`}>
                                                    {fromInfo.badgeLabel || '—'}
                                                </span>
                                            </DetailRow>
                                        </div>
                                        ...
                                        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                                            <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Paid by</p>
...
const ViewPayments = () => {
    ...
    const prepareExportData = () => {
        ...
        const columns = [
            ...
            { header: 'Paid From', key: 'paid_from', width: 20 },
        ...
        payments.forEach((item, index) => {
            const partyInfo = item.payment_to?.details?.name || item.payment_to?.type || 'N/A';
            const fromInfo = getPaymentFromInfo(item).displayName;
    ...
    const fetchPaymentsData = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${API_BASE_URL}/transaction/report/payment?${params.toString()}`,
                { method: 'GET', headers }
            );
            ...
            if (result.success) {
                ...
                setStats({
                    count: Number(result.stats?.count ?? result.meta?.total) || 0,
    ...
    const activePaymentItem = useMemo(
        () => payments.find((item) => item.transaction_id === activeRowDropdown) || null,
        [payments, activeRowDropdown]
    );
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (
                !event.target.closest('[data-payment-actions-menu]') &&
                !event.target.closest('[data-payment-actions-trigger]')
            ) {
                setActiveRowDropdown(null);
                actionAnchorRef.current = null;
            }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white">
                                    {listLoading ? (
                                        ...
                                        payments.map((item, index) => {
                                            const partyInfo = getPaymentToInfo(item);
                                            const paidFromInfo = getPaymentFromInfo(item);
            ...
            {activeRowDropdown && activePaymentItem && createPortal(
                <motion.div
                ...
                >
                    <button
                        ...
                        onClick={() => openDetails(activePaymentItem)}
                    >
                    ...
                    <button
                        ...
                        onClick={() => {
                            ...
                            openEditModal(activePaymentItem);
                        }}
                    >
                        <FiEdit className="mr-2 h-4 w-4 text-blue-500" />
                        Edit Payment
                    </button>
                    {getActionLinks(activePaymentItem).invoiceLink ? (
                        <a
                            href={getActionLinks(activePaymentItem).invoiceLink}
