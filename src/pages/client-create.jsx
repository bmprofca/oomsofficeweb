...
const CreateClient = () => {
  ...
  useEffect(() => {
    ...
    const fetchFirmTypes = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/utils/firm-types`, {
          headers,
        });
        ...
        if (
          ...
          setFirmTypes(response.data.data);
        }
      } catch (error) {
  ...
  const [states] = useState([
    ...
    {
      state: "West Bengal",
      districts: ["Hooghly", "Kolkata", "Howrah", "North 24 Parganas"],
    },
  ]);
  ...
  const getBusinessDistricts = () => {
    const selectedState = states.find(
      (state) => state.state === formData.businesses[0]?.address?.state,
    );
    return selectedState ? selectedState.districts : [];
  };
    ...
    if (type === "file") {
      ...
      if (name === "mobile") {
        ...
        setErrors((prev) => ({
          ...prev,
          mobile:
            value && !mobileRegex.test(value) ? "Invalid mobile number" : "",
        }));
      }
      ...
      if (name === "gst") {
        ...
        setErrors((prev) => ({
          ...prev,
          gst:
            value && !gstRegex.test(value.toUpperCase())
              ? "Invalid GST format"
              : "",
        }));
      }
  ...
  const selectAllGroups = () => {
    setGroupsList((prev) =>
      prev.map((group) => ({ ...group, selected: true })),
    );
    setFormData((prev) => ({
      ...prev,
      businesses: prev.businesses.map((business, index) =>
  ...
  const deselectAllGroups = () => {
    setGroupsList((prev) =>
      prev.map((group) => ({ ...group, selected: false })),
    );
    setFormData((prev) => ({
      ...prev,
      businesses: prev.businesses.map((business, index) =>
  ...
  const validateForm = () => {
    ...
    if (currentStep === 1) {
      ...
      else if (panDuplicateClient)
        newErrors.pan = "A client already exists with this PAN";
    ...
    if (currentStep === 2) {
      ...
      if (!formData.town_or_village.trim())
        newErrors.town_or_village = "Town/Village is required";
    ...
    if (currentStep === 3) {
      formData.businesses.forEach((business, index) => {
        if (!business.pan.trim())
          newErrors[`business_pan_${index}`] = "Business PAN is required";
    ...
    // Step 4 validation
    if (currentStep === 4) {
      if (!formData.opening_balance.amount)
        newErrors.opening_amount = "Opening balance amount is required";
      if (!formData.opening_balance.date)
        newErrors.opening_date = "Date is required";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (panDuplicateClient) {
      ...
      alert(
        "A client already exists with this PAN. Please use a different PAN.",
      );
      return;
    }
    ...
    try {
      ...
      if (error.response) {
        ...
        switch (status) {
          ...
          case 403:
            errorMessage =
              "Forbidden. You do not have permission to create clients.";
            break;
          case 409:
            errorMessage =
              data.message || "Client with this PAN or email already exists.";
            break;
          case 422:
            errorMessage =
                      ...
                      <div className="absolute left-0 top-full mt-2 hidden group-hover:block w-72 sm:w-80 p-4 bg-slate-900 text-slate-100 rounded-lg shadow-xl border border-slate-700 text-xs font-normal leading-relaxed z-50">
                        ...
                        <div className="space-y-2">
                          <p>
                            ...
                            Always enter the client's{" "}
                            <span className="font-semibold text-indigo-300">
                              PAN Number
                            </span>{" "}
                            first. This validates the identity format and
                            prepares the form.
                          </p>
                          <p>
                            <span className="font-semibold text-white">
                            ...
                            button on the right to upload multiple clients via
                            Excel/CSV templates.
                          </p>
                        </div>
                      </div>
                ...
                <div className="flex items-center space-x-6">
                  {steps.map((step, index) => (
                    <div key={step.number} className="flex items-center">
                      ...
                      >
                        <div
                          className={`relative flex items-center justify-center w-10 h-10 rounded-full text-sm font-semibold border-2 transition-all duration-300 ${
                            step.number === currentStep
                                      ...
                                      <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                        ...
                                        {filteredBusinessStates.map((state) => (
                                          <div
                                            ...
                                            className="px-4 py-2 text-sm hover:bg-indigo-50 cursor-pointer flex items-center justify-between"
                                            onClick={() =>
                                              selectBusinessState(state.state)
                                            }
                                          >
                                            <span>{state.state}</span>
                                            {business.address.state ===
                                ...
                                <div className="space-y-1 dropdown-container">
                                  ...
                                  <div className="relative">
                                    <div
                                      ...
                                      onClick={() =>
                                        business.address.state &&
                                        toggleDropdown("businessDistrict")
                                      }
                                    >
                                      <span
                                        className={
                                    ...
                                    {isDropdownOpen.businessDistrict &&
                                      business.address.state && (
                                        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                          <div className="sticky top-0 bg-white p-2 border-b">
                                            <div className="relative">
                            ...
                            <div className="space-y-2 dropdown-container">
                              ...
                              <div className="relative">
                                ...
                                >
                                  ...
                                  >
                                    {business.groups.length > 0
                                      ? `${business.groups.length} group(s) selected`
                                      : "Select groups"}
                                  </span>
                                  <FiSearch className="w-4 h-4 text-gray-400" />
                                </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleOpeningBalanceChange("type", e.target.value)
                            }
                            className="w-full p-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white outline-none transition-colors duration-200"
                            required
                          >
                ...
                <div className="py-8 flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      ...
                      <strong>{bulkSuccessResult.imported_count}</strong>{" "}
                      clients.
                    </p>
                    ...
                    {bulkFile ? (
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          {(bulkFile.size / 1024).toFixed(1)} KB • Click or drag
                          to replace
                        </p>
                      </div>
                    ) : (
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 md:p-5 space-y-4 animate-fade-in">
                        <div>
                          ...
                          <p className="text-[11px] text-slate-505 mt-1">
                            Map the expected client profile details to the
                            headers in your uploaded spreadsheet. Required
                      ...
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 border-b border-slate-200/60 pb-2">
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 mt-0.5">
                            Reference headers & templates below
                          </p>
                        </div>
          ...
          >
            <div className="bg-gradient-to-r from-amber-500 to-orange-600 px-4 py-3 text-white flex justify-between items-center shrink-0">
              <div className="min-w-0">
                <h3
                  id="existing-pan-modal-title"
                  className="text-sm font-bold truncate"
                >
                  Existing Client
                </h3>
                <p className="text-[11px] text-amber-100 truncate">
                  PAN already registered in this branch
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="px-4 py-3 shrink-0">
              <div className="flex items-center gap-3 mb-3 pb-3 border-b border-slate-100">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-slate-900 leading-tight break-words">
                    {panDuplicateClient.name || "-"}
                  </p>
                  {(panDuplicateClient.care_of ||
                    panDuplicateClient.guardian_name) && (
              ...
              <dl className="grid grid-cols-2 gap-x-3 gap-y-2.5 text-xs">
                <div className="min-w-0">
                  ...
                  <dd className="font-mono font-medium text-slate-800 mt-0.5 break-all">
                    {formData.pan || "-"}
                  </dd>
                </div>
                <div className="min-w-0">
                  ...
                  <dd className="font-medium text-slate-800 mt-0.5 break-all">
                    {panDuplicateClient.username || "-"}
                  </dd>
                </div>
                <div className="min-w-0">
                  <dt className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="rounded-xl border border-slate-200 p-3">
                  ...
                  <p className="text-sm font-medium text-slate-800 mt-1 break-all">
                    {clientCreateSuccess.profile_id || "-"}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-200 p-3">
                ...
                <div className="rounded-xl border border-slate-200 p-3">
                  ...
                  <p className="text-sm font-medium text-slate-800 mt-1 font-mono">
                    {clientCreateSuccess.pan_number || "-"}
                  </p>
                </div>
                <div className="rounded-xl border border-slate-200 p-3">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400 flex items-center gap-1">
                  ...
                  <p className="text-sm font-medium text-slate-800 mt-1">
                    {clientCreateSuccess.branch_id || "-"}
                  </p>
                </div>
              </div>

              {Array.isArray(clientCreateSuccess.firms) &&
                clientCreateSuccess.firms.length > 0 && (
