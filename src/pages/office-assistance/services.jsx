...
const isComplianceService = (svc) =>
  String(svc?.type || "").toLowerCase() === "compliance";
...
const frequencyBadge = (frequency) => {
  ...
  const cls =
    FREQ_BADGE_COLORS[key] || "bg-gray-100 text-gray-600 border-gray-200";
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold border uppercase tracking-wide ${cls}`}
    >
      {frequency}
    </span>
  );
...
const isServiceOnBranch = (svc) => {
  if (svc && "is_added" in svc)
    return svc.is_added === true || svc.is_added === 1 || svc.is_added === "1";
  if (svc && "added_to_branch" in svc) {
    return (
      svc.added_to_branch === true ||
      svc.added_to_branch === 1 ||
      svc.added_to_branch === "1"
    );
  }
  return false;
...
const FEES_CHIP =
  "inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-200";
    ...
    <ViewportTooltip label={added ? "Added to branch" : "Not added"}>
      ...
      >
        {added ? (
          ...
          <FiX className="w-3.5 h-3.5" strokeWidth={2.5} />
        )}
      </span>
    </ViewportTooltip>
  );
...
const typeBadge = (type) => {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-semibold border ${cls} uppercase tracking-wide`}
    >
      {label}
    </span>
  );
...
const ActionMenu = ({ items }) => {
  ...
  const calcPos = useCallback(() => {
    ...
    const top =
      r.bottom + GAP + mH <= vh - PAD ? r.bottom + GAP : r.top - mH - GAP;
...
const FeeForm = ({
  ...
  const dueMeta = getDueDateFieldMeta(frequency);
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-gray-500">
        GST is applied automatically by the server when this branch is
        GST-applicable.
      </p>
      ...
      {showDueDate ? (
        <div>
          ...
          />
          <p className="text-[10px] text-gray-400 mt-1">{dueMeta.help}</p>
        </div>
      ) : null}
      ...
      <div className="shrink-0 flex items-center justify-between gap-3 px-5 py-3.5 bg-gradient-to-r from-indigo-600 to-violet-600">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {subtitle && (
              <p className="text-xs text-indigo-200 leading-tight mt-0.5 truncate">
                {subtitle}
              </p>
            )}
          </div>
  ...
  return (
    ...
    >
      <div className="mb-4 p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
        <h4 className="text-sm font-bold text-indigo-900 leading-snug">
          {svc.name}
        </h4>
        <div className="flex items-center gap-2 mt-2 flex-wrap">
          {typeBadge(svc.type)}
          {svc.sac_code && (
            <span className="text-[11px] font-mono text-indigo-600 bg-indigo-100 px-1.5 py-0.5 rounded">
              {svc.sac_code}
            </span>
          )}
      ...
      {!onBranch &&
        isComplianceService(svc) &&
        (svc.frequency || svc.default_due_date != null) && (
            ...
            {svc.default_due_date != null && (
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5">
                ...
                <p className="text-sm font-bold text-gray-800">
                  {formatDueOffset(svc.default_due_date)}
                </p>
              </div>
            )}
          </div>
      ...
      {hasFees && (
        <div className="grid grid-cols-2 gap-3 mb-4">
          {[
            ...
            {
              label: "GST Rate",
              value: `${svc.gst_rate ?? 0}%`,
              cls: "bg-gray-50 border-gray-200",
            },
            {
              label: "GST Value",
              value: `₹ ${fmt(svc.gst_value)}`,
              cls: "bg-amber-50 border-amber-100",
            },
            {
              label: "Total",
              value: `₹ ${fmt((parseFloat(svc.fees) || 0) + (parseFloat(svc.gst_value) || 0))}`,
      ...
      {getServiceRemark(svc) && (
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-700 bg-gray-50 border border-gray-100 rounded-lg px-3 py-2.5 leading-relaxed">
            {getServiceRemark(svc)}
          </p>
        </div>
      )}
      ...
      {(svc.modify_by?.name || svc.modify_date) && (
        <div className="pt-3 border-t border-gray-100">
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            {svc.modify_date && (
              <span>
                {new Date(svc.modify_date).toLocaleDateString("en-IN", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                })}
...
const Services = () => {
  ...
  const getStaffNames = useCallback(
    (employee_username) => {
      ...
      const usernames =
        typeof employee_username === "string"
          ? employee_username.split(",").map((u) => u.trim())
          : Array.isArray(employee_username)
            ? employee_username
            : [employee_username];
  ...
  const fetchFirmsForService = useCallback(
    async (serviceId, search = "", page = 1, limit = 20) => {
      ...
      try {
        ...
        toast.error(
          err.response?.data?.message || "Error loading assigned firms",
        );
      } finally {
  ...
  useEffect(() => {
    ...
    firmsTimer.current = setTimeout(() => {
      setFirmsPage(1);
      fetchFirmsForService(
        selectedServiceForFirms.service_id,
        firmsSearch,
        1,
        firmsLimit,
      );
  ...
  const handleEdit = async () => {
    ...
    if (isCompliance) {
      const dueMeta = getDueDateFieldMeta(editTarget?.frequency);
      if (
        editForm.due_date === "" ||
        !isValidDueDateOffset(editForm.due_date, editTarget?.frequency)
      ) {
        toast.error(dueMeta.error);
        return;
      }
      payload.due_date = Number(editForm.due_date);
    } else {
    ...
    setEditLoading(true);
    try {
      const res = await axios.put(`${API_BASE_URL}/service/edit`, payload, {
        headers: getHeaders(),
      });
      ...
      toast.error(apiMessage(err.response?.data, "Error updating service"));
    } finally {
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      toast.error(apiMessage(err.response?.data, "Error removing service"));
    } finally {
  ...
  const openAdd = (svc) => {
    ...
    setAddForm({
      fees: "",
      due_date:
        isCompliance && svc.default_due_date != null
          ? String(svc.default_due_date)
          : "",
      remark: "",
    });
  };
  ...
  const handleAdd = async () => {
    ...
    if (isCompliance) {
      if (addForm.due_date !== "") {
        const dueMeta = getDueDateFieldMeta(addTarget?.frequency);
        if (!isValidDueDateOffset(addForm.due_date, addTarget?.frequency)) {
          toast.error(dueMeta.error);
          return;
        }
        payload.due_date = Number(addForm.due_date);
      }
    ...
    setAddLoading(true);
    try {
      const res = await axios.post(`${API_BASE_URL}/service/add`, payload, {
        headers: getHeaders(),
      });
      if (isApiSuccess(res.data)) {
      ...
      toast.error(apiMessage(err.response?.data, "Error adding service"));
    } finally {
      setAddLoading(false);
    }
  };
  ...
  const dueDateValid =
    !isComplianceAdd ||
    addForm.due_date === "" ||
    isValidDueDateOffset(addForm.due_date, addTarget?.frequency);
  const isAddFormValid = !!addTarget && feesValid && dueDateValid;
                  ...
                  {viewMode === "firms" ? (
                    <>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5 truncate">
                        Firms assigned to {selectedServiceForFirms?.name}
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        Manage services enabled for this branch
                      </p>
                    </>
                  )}
                </div>
              ...
              <div>
                <div className={TOOLBAR_ROW}>
                  <div className="relative flex-1 max-w-xs">
                    ...
                    <input
                      type="text"
                      placeholder="Search assigned firms…"
                      value={firmsSearch}
                  ...
                  <ViewportTooltip label="Refresh">
                    ...
                    >
                      <FiRefreshCw
                        className={`w-3.5 h-3.5 ${firmsLoading ? "animate-spin" : ""}`}
                      />
                    </button>
                  </ViewportTooltip>
                </div>
                    ...
                    <tbody className="bg-white">
                      {firmsLoading ? (
                        ...
                        <EmptyState
                          ...
                          title="No assigned firms found"
                          desc="No firms are currently assigned to this service."
                        />
                      ) : (
                          ...
                          >
                            ...
                            <td className={`${TABLE_TD} max-w-[220px]`}>
                              <button
                                onClick={() =>
                                  navigate(
                                    `/client/profile/${firm.client?.username}/firms`,
                                  )
                                }
                            ...
                            <td className={TABLE_TD}>
                              <p
                                className={`${CELL_BODY} font-semibold text-gray-800`}
                              >
                                {firm.gst_no || "—"}
                              </p>
                            ...
                            <td className={TABLE_TD}>
                              <p
                                className={`${CELL_BODY} ${!check("recurring_task_fees_view") ? "blur-[3.5px] select-none" : ""}`}
                              >
                                Fees:{" "}
                                {check("recurring_task_fees_view")
                                  ? `₹${fmt(firm.assignment?.custom_amount)}`
                                  : "₹••••"}
                              </p>
                              <p className="text-xs text-gray-500 mt-0.5">
                                Pay Month:{" "}
                                {firm.assignment?.pay_from_month ||
                                  firm.assignment?.period_name ||
                                  "—"}
                              </p>
                              <p className="text-xs text-gray-400 mt-0.5">
                                Staff:{" "}
                                {getStaffNames(
                                  firm.assignment?.employee_username,
                                )}
                              </p>
                            </td>
                            <td className={TABLE_TD}>
                ...
                <div className="border-t border-gray-200 px-3 md:px-4 py-2">
                  <TablePagination
                    ...
                    isLastPage={firmsPage >= firmsTotalPages}
                    onPageChange={(p) => {
                      setFirmsPage(p);
                      fetchFirmsForService(
                        selectedServiceForFirms.service_id,
              ...
              <>
                <div className="flex border-b border-gray-200 px-3 md:px-4 pt-3 gap-1">
                  {SERVICE_CATEGORIES.map((tab) => (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium rounded-t-lg border-b-2 transition-all -mb-px ${
                ...
                <div className={TOOLBAR_ROW}>
                  ...
                  <ViewportTooltip label="Refresh">
                    ...
                    >
                      <FiRefreshCw
                        className={`w-3.5 h-3.5 ${serviceLoading ? "animate-spin" : ""}`}
                      />
                    </button>
                  </ViewportTooltip>
                </div>
                  ...
                  <table className="w-full table-fixed min-w-[850px]">
                    <thead>
                      <tr className={TABLE_HEAD_ROW}>
                        ...
                        {serviceCategory === "compliance" && (
                          <th className={`${TABLE_TH} w-28`}>Due Offset</th>
                        )}
                        {serviceCategory === "compliance" && (
                    ...
                    <tbody className="bg-white">
                      {serviceLoading ? (
                        Array.from({ length: 6 }).map((_, i) => (
                          <SkeletonRow
                            key={i}
                            cols={serviceCategory === "compliance" ? 9 : 6}
                          />
                        ))
                        ...
                        <EmptyState
                          ...
                          desc={
                            serviceSearch
                              ? "No match found."
                              : showAddedOnly
                                ? "No services are added to this branch yet."
                              ...
                              <td className={`${TABLE_TD} max-w-[200px]`}>
                                <div className="flex flex-col items-start gap-0.5 min-w-0">
                                  <ViewportTooltip label={svc.name} fullWidth>
                                    <p
                                      className={`${CELL_TITLE} leading-tight truncate`}
                                    >
                                      {svc.name}
                                    </p>
                                  </ViewportTooltip>
                                  {getServiceRemark(svc) && (
                                    <p
                                      className="text-xs text-gray-400 leading-tight line-clamp-2"
                                      title={getServiceRemark(svc)}
                                    >
                                      {getServiceRemark(svc)}
                                    </p>
                                  )}
                                  {svc.sac_code && (
                                    <span className="text-xs font-mono text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded inline-block">
                                      {svc.sac_code}
                                    </span>
                                  )}
                                </div>
                                ...
                                <td className={TABLE_TD}>
                                  {onBranch && svc.due_date != null ? (
                                    ...
                                    <span
                                      ...
                                      title="Default due offset from period end"
                                    >
                                      {formatDueOffset(svc.default_due_date)}{" "}
