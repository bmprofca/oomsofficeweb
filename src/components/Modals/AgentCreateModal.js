...
export default function AgentCreateModal({ isOpen, onClose, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const message = err.response?.data?.message || err.message || 'Failed to create agent';
            if (status === 409) {
                setFormError(message || 'An agent with this mobile, email, or PAN already exists in this branch.');
            } else if (status === 400) {
