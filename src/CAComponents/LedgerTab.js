...
export default function LedgerTab({
    ...
    useEffect(() => {
        if (!caUsername) {
            toast.error('CA username not found');
        }
    }, [caUsername]);
    ...
    const fetchTransactions = useCallback(async () => {
        ...
        try {
            const response = await axios.get(
                `${API_BASE_URL}/transaction/list?page_no=${currentPage}&limit=${itemsPerPage}&from_date=${fromDate}&to_date=${toDate}&party_type=ca&party_id=${encodeURIComponent(caUsername)}`,
                { headers: getHeaders() }
            );
    ...
    }, [caUsername, fromDate, toDate, currentPage, itemsPerPage]);
    ...
    const fetchOpeningBalance = useCallback(async () => {
        ...
        try {
            const res = await axios.get(
                `${API_BASE_URL}/transaction/get-opening-balance?party_type=ca&party_id=${encodeURIComponent(caUsername)}`,
                { headers: getHeaders() }
            );
            ...
            >
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="min-w-0">
                        <h2 className="text-base font-bold text-slate-800 sm:text-lg">CA Ledger</h2>
