...
const BasicDetailsTab = ({
    ...
    const fetchUtils = async () => {
      try {
        ...
        const [careOfRes, statesRes] = await Promise.all([
          ...
          axios.get(
            `${API_BASE_URL}/utils/states-and-districts`,
            requestConfig,
          ),
        ]);
  ...
  const openEditModal = (section) => {
    const sourceFields =
      section === "personal" ? personalFields : addressFields;
    const initialData = {};
    sourceFields.forEach(({ key }) => {
      ...
      initialData[key] =
        clientData[key] ?? (key === "country_code" ? "91" : "");
    });
  ...
  const updateClientProfile = async (updatedData) => {
    ...
    if (!headers) {
      setSaveStatus({
        type: "error",
        message: "Authentication headers missing. Please login again.",
      });
      return false;
    }
    ...
    try {
      ...
      if (err.response) {
        errorMessage =
          ...
          `Error ${err.response.status}: Update failed`;
      } else if (err.request) {
  ...
  const renderValue = (field) => {
    if (field.key === "is_active")
      return clientData[field.key] ? "Active" : "Inactive";
  ...
  const modalFields =
    activeModal === "personal" ? personalFields : addressFields;
  const selectedStateName = formData.state || clientData.state || "";
  const selectedStateData = statesAndDistricts.find(
    (stateItem) => stateItem.name === selectedStateName,
  );
          ...
          <div className="flex flex-wrap items-center justify-between gap-2.5">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Compact client information overview
              </p>
            </div>
            {saveStatus && (
              <div
                className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium ${saveStatus.type === "success" ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
              >
          ...
          >
            ...
            >
              <div className="sticky top-0 z-10 shrink-0 flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
                <h4 className="text-sm font-semibold text-slate-900">
                  Edit{" "}
                  {activeModal === "personal" ? "Personal Details" : "Address"}
                </h4>
                <button
                  type="button"
                  onClick={closeModal}
                      ...
                      {field.key === "care_of" ? (
                          ...
                          onChange={(e) => {
                            ...
                            setFormData((prev) => {
                              const nextDistrictOptions =
                                statesAndDistricts.find(
                                  (s) => s.name === nextState,
                                )?.districts || [];
                              const shouldResetDistrict =
                                !nextDistrictOptions.includes(
                                  prev.district || "",
                                );
                              return {
                                ...prev,
                                state: nextState,
                                district: shouldResetDistrict
                                  ? ""
                                  : prev.district || "",
                              };
                            });
                        ...
                        activeModal === "personal" ? (
                          ...
                          >
                            ...
                            <span
                              className={`mt-1 block text-left text-xs font-semibold ${formData[field.key] ? "text-emerald-700" : "text-slate-600"}`}
                            >
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-blue-100 text-xs mt-1">
                Update the {field.toLowerCase()} information
              </p>
            </div>
            <button
              onClick={onClose}
...
/** Full-page skeleton matching client profile header + tabs + content layout. */
const ClientProfilePageSkeleton = ({ tabsMinimized = true, tabCount = 12 }) => {
  const tabPlaceholders = Array.from({ length: tabCount }, (_, i) => i);

  return (
    <div aria-busy="true" aria-label="Loading client profile">
      {/* Profile summary — mirrors live header card */}
      <section
      ...
      {/* Tabs bar — mirrors minimized / expanded chrome */}
      ...
      {/* Tab content — mirrors Basic Details card layout (default landing tab) */}
...
const ClientProfile = () => {
    ...
    async (currentUsername, options = {}) => {
      ...
      try {
        ...
        if (response.data.success && response.data.data) {
          ...
          setClientData({
            // Basic info
            username: apiData.basic?.username || usernameToFetch,
            name: apiData.basic?.name || "",
            care_of: apiData.basic?.care_of || "",
            guardian_type:
              apiData.basic?.guardian_type || apiData.basic?.care_of || "",
            guardian_name: apiData.basic?.guardian_name || "",
            date_of_birth: apiData.basic?.date_of_birth
              ? new Date(apiData.basic.date_of_birth).toLocaleDateString(
                  "en-GB",
                )
              : "",
        ...
        if (err.response) {
          if (err.response.status === 401) {
            ...
            setError(
              `Error ${err.response.status}: ${err.response.data?.message || "Failed to fetch client data"}`,
            );
          }
        } else if (err.request) {
  ...
  // Format date for display (handles dd/mm/yyyy, yyyy-mm-dd, and ISO strings)
  const formatDate = (dateString) => {
    if (!dateString) return "";
    const value = String(dateString).trim();
    // Already in dd/mm/yyyy display format
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50/30">
      ...
      >
        <div className="w-full px-2 sm:px-4 md:px-8 py-4 md:py-6">
          ...
          >
            {/* Loading State — layout-matching skeleton */}
            {loading && (
              <ClientProfilePageSkeleton
                            ...
                            <button
                              ...
                              onClick={() =>
                                setClientPaymentReminder({
                                  ...
                                  clients: [
                                    {
                                      username: clientData.username || username,
                              ...
                              className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-fuchsia-600 text-white shadow-sm shadow-violet-200 transition hover:brightness-110"
                              title="Send payment reminder"
                              aria-label={`Send payment reminder to ${clientData.name || username}`}
                            >
      ...
      <ClientPaymentReminderModal
        ...
        onSuccess={() => fetchClientData(username, { silent: true })}
