...
import { editAgentProfile } from '../services/agentService';
...
export default function ProfileTab({ agentData, onEdit, loading, agentUsername, onRefresh }) {
    ...
    const openEditModal = (section) => {
        ...
        sourceFields.forEach(({ key }) => {
            if (key === 'date_of_birth') {
                initialData[key] = formatDateForInput(agentData.date_of_birth);
                return;
            }
            initialData[key] = agentData[key] ?? (key === 'country_code' ? '91' : '');
        });
    ...
    const updateAgentProfile = async (updatedData) => {
        ...
        try {
            setIsSaving(true);
            const result = await editAgentProfile(requestBody);
            if (!result?.success) {
    ...
    const handleModalSave = async () => {
        ...
        fieldKeys.forEach((key) => {
            payload[key] = formData[key] ?? agentData[key] ?? '';
        });

        const success = await updateAgentProfile(payload);
        if (!success) return;
    ...
    const renderValue = (field) => {
        if (field.key === 'is_active') return agentData[field.key] ? 'Active' : 'Inactive';
        if (field.key === 'date_of_birth') return formatDateDisplay(agentData[field.key]);
        if (field.key === 'gender') {
            const g = agentData[field.key];
            return g ? String(g).charAt(0).toUpperCase() + String(g).slice(1) : 'Not provided';
        }
        return agentData[field.key] || 'Not provided';
    };
    ...
    const selectedStateName = formData.state || agentData.state || '';
    ...
    const currentImage = formData.image || agentData.image || '';
                ...
                <div className="border-b border-slate-200 px-4 py-3">
                    <div className="flex flex-wrap items-center justify-between gap-2.5">
                        <div>
                            ...
                            <p className="text-xs text-slate-500">Agent information overview</p>
                        </div>
                        {saveStatus && (
                            <div
