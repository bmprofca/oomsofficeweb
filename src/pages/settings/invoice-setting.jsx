...
// Use CDN worker (same pattern as OneChattingLiveChat). Bundled `.mjs` workers via
// `import.meta.url` often break in production (wrong MIME type / SPA 404 HTML),
// which makes react-pdf fail while plain fetch/download still works.
pdfjs.GlobalWorkerOptions.workerSrc = `//unpkg.com/pdfjs-dist@${pdfjs.version}/build/pdf.worker.min.mjs`;

/** Fetch remote PDF (or decode base64) into a same-origin blob URL for react-pdf.
 *  pdf.js cannot reliably load cross-origin `/media/...` URLs in production (CORS),
 *  but `fetch` + blob works — same approach as download.
 */
async function toLocalPdfBlobUrl(sample) {
  if (sample?.url) {
...
const FormatCard = memo(function FormatCard({
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!sample?.url && !sample?.data) {
        setThumbError(true);
        return;
      }
      try {
        const url = await toLocalPdfBlobUrl(sample);
        if (cancelled) {
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            >
              <FiFileText
                className={`w-6 h-6 ${isActive ? "text-green-600" : "text-indigo-600"}`}
              />
            </div>
        ...
        <div
          ...
          style={{ height: "200px" }}
        >
          {sample.url || sample.data ? (
            !thumbError ? (
              <>
                {(!thumbLoaded || !thumbFile) && (
...
const InvoiceSettings = () => {
  ...
  // Prefix types (invoice numbering) — broader set used by vouchers across the app
  const invoicePrefixTypes = [
  ...
  // PDF format / generation — only these types are supported
  ...
  const fetchFormatData = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/invoice/formats?type=${selectedFormatType}`,
        {
          method: "GET",
          headers,
        },
      ...
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error(
          "Server returned non-JSON response. Please check if the endpoint is correct.",
        );
      }
  ...
  const handleActivateFormat = async (formatId) => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error(
          "Server returned non-JSON response. Please check if the update-format endpoint is correct.",
        );
      }
      ...
      toast.error(
        error.message ||
          "Failed to activate format. Please check if the endpoint is correct.",
      );
    } finally {
  ...
  const handlePreviewFormat = async (sample) => {
    ...
    try {
      // Always materialize a same-origin blob so pdf.js does not hit cross-origin CORS.
      const url = await toLocalPdfBlobUrl(sample);
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const isAlreadyLocal =
        previewPdfUrl.startsWith("blob:") || previewPdfUrl.startsWith("data:");
      ...
      toast.error(
        "Failed to download PDF. It may be blocked by CORS — try again or contact support.",
      );
    } finally {
  ...
  const handleCreateInvoicePrefix = async (e) => {
    ...
    if (!String(formData.prefix || "").trim()) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                  ...
                  <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <div>
                      ...
                      <p className="text-gray-500 text-xs">
                        {filteredInvoices.length} of {invoicePrefixData.length}{" "}
                        invoice prefixes shown
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-3 w-full lg:w-auto">
                      ...
                      <div className="flex gap-2 items-start min-w-[220px]">
                        ...
                        <motion.button
                          ...
                          className="px-4 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 shadow-sm whitespace-nowrap"
                          ...
                          return (
                            <tr
                              ...
                              className={`transition-colors ${
                                active
                                  ? "hover:bg-gray-50"
                                  : "bg-gray-50/60 hover:bg-gray-100/60 opacity-80"
                              }`}
                            >
                              ...
                              <td className="p-4">
                                <div className="flex items-center gap-2 text-sm text-gray-700">
                                  ...
                                  <FiCalendar className="w-3 h-3 text-gray-400 flex-shrink-0" />
                                  <span>
                                    {formatDateForDisplay(invoice.expire_date)}
                                  </span>
                                </div>
                              </td>
                              <td className="p-4 text-center">
                ...
                <div className="border-b border-gray-200 px-6 py-4">
                  <div className="flex flex-col gap-4">
                    <div>
                      ...
                      <p className="text-gray-500 text-xs">
                        Manage and activate different invoice formats for each
                        invoice type
                      </p>
                    </div>

                    <div className="flex flex-wrap gap-2">
                      {invoiceFormatTypes.map((type) => {
                        const isSelected = selectedFormatType === type.value;
                        return (
                          <button
                            key={type.value}
                            type="button"
                            onClick={() => setSelectedFormatType(type.value)}
                ...
                <div className="p-6">
                  {formatLoading ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                      {(formatData.samples || []).map((sample) => (
                        <FormatCard
                          key={sample.format_id}
                          sample={sample}
                          isActive={
                            formatData.active_format === sample.format_id
                          }
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white px-6 py-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-indigo-100 text-sm mt-1">
                  Configure new invoice prefix settings
                </p>
              </div>
              <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <CustomSelect
                        label="Invoice Type"
                        required
                        options={invoicePrefixTypes}
                        value={
                          invoicePrefixTypes.find(
                            (o) => o.value === formData.type,
                          ) || null
                        }
                        onChange={(opt) =>
                          handleInputChange("type", opt?.value || "")
                        }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleInputChange("expire_date", e.target.value)
                        }
                        className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all duration-200"
                        required
                      />
      ...
      {showPreviewModal && selectedFormatSample && (
        <div className="fixed inset-0 bg-black bg-opacity-95 z-50 flex flex-col">
          ...
          <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white px-6 py-2 flex justify-between items-center shadow-lg">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={downloadingPdf || !previewPdfUrl}
              ...
              >
                <FiDownload
                  className={`w-5 h-5 ${downloadingPdf ? "animate-pulse" : ""}`}
                />
              </button>
              <button
          ...
          <div
            ...
            className="flex-1 bg-gray-900 flex flex-col overflow-hidden"
          >
            ...
            <div className="flex items-center justify-center gap-3 bg-gray-800 border-b border-gray-700 py-2 px-4">
              ...
              <span className="text-sm text-gray-200 w-12 text-center select-none">
                {Math.round(zoom * 100)}%
              </span>
            ...
            <div className="flex-1 overflow-auto flex justify-center p-4">
              {previewLoading ? (
                <p className="text-gray-300 text-sm mt-16">
                  Loading preview...
                </p>
              ) : pdfLoadError || !previewPdfUrl ? (
