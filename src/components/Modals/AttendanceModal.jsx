...
const STATE_BADGE = {
  not_punched: 'bg-slate-100 text-slate-700',
  punched_in: 'bg-emerald-50 text-emerald-700',
  on_break: 'bg-amber-50 text-amber-800',
  punched_out: 'bg-rose-50 text-rose-700',
};
  ...
  return (
    <div aria-busy="true" aria-live="polite">
      ...
      <div className="relative">
        <div className="absolute bottom-2 left-[4px] top-2 w-px bg-slate-200" aria-hidden="true" />
        <ul className="m-0 list-none space-y-0 p-0">
          {[0, 1, 2].map((row) => (
            <li key={row} className={`relative flex gap-3 ${row < 2 ? 'pb-4' : ''}`}>
              <span className="relative z-[1] mt-1 inline-flex h-2.5 w-2.5 shrink-0 rounded-full bg-slate-200 ring-4 ring-slate-100" />
              <div className={`min-w-0 flex-1 ${row < 2 ? 'border-b border-slate-100 pb-4' : ''}`}>
                <div className="flex items-center justify-between gap-2">
                  <SkeletonBlock className="h-4 w-20" />
                  <SkeletonBlock className="h-4 w-12" />
                </div>
                {row === 1 ? <SkeletonBlock className="mt-2 h-3.5 w-28" /> : null}
              </div>
            </li>
          ))}
        </ul>
...
function SwipeToConfirm({
  ...
  const measure = useCallback(() => {
    ...
    const nextMax = Math.max(0, trackRef.current.clientWidth - THUMB_SIZE - TRACK_PAD * 2);
  ...
  const endDrag = useCallback(() => {
    ...
    if (!confirmedRef.current && maxOffset > 0 && current >= maxOffset * CONFIRM_RATIO) {
  ...
  const onPointerMove = (event) => {
    ...
    const next = Math.min(maxOffset, Math.max(0, startOffsetRef.current + delta));
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-xs font-semibold tracking-wide">{swipeLabel}</span>
      ...
      {progress > 0.55 && !loading ? (
        <div className={`pointer-events-none absolute inset-0 flex items-center justify-center ${text}`}>
      ...
      <button
        ...
        style={{
          ...
          transitionProperty: dragging ? 'box-shadow, transform' : 'left, box-shadow, transform',
          transitionDuration: dragging ? '0ms' : '180ms',
        }}
        aria-label={swipeLabel}
      >
        {loading ? <FiLoader className="h-5 w-5 animate-spin" /> : <Icon className="h-5 w-5" />}
      </button>
    </div>
  );
}
...
const AttendanceModal = ({ isOpen, onClose, branchName, branchId }) => {
  ...
  const displayBranch =
    branchName ||
    (typeof window !== 'undefined' ? localStorage.getItem('branch_name') : '') ||
    '—';
  ...
  const runAction = useCallback(async (actionKey) => {
    ...
    try {
      ...
      const result = await res.json().catch(() => ({}));
      if (!res.ok || !result?.success) {
        throw new Error(result?.message || meta.success || 'Attendance action failed');
      }
      setStatus(result.data || null);
      setPendingAction(null);
            ...
            <header className="shrink-0 border-b border-slate-100 px-5 py-3.5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h2 id="attendance-modal-title" className="m-0 text-base font-semibold text-slate-900">
                    Attendance
                  </h2>
                  {showSkeleton ? (
                ...
                <>
                  <div className="mb-4 flex items-center justify-between gap-3 rounded-xl border border-slate-200 px-3 py-2.5">
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-500">
                      <span>
                        Work <strong className="font-semibold text-slate-800">{workDuration}</strong>
                      </span>
                      <span className="text-slate-300">|</span>
                      <span>
                        Break <strong className="font-semibold text-slate-800">{totalBreakDuration}</strong>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <ul className="m-0 list-none space-y-0 p-0">
                      <li className="relative flex gap-3 pb-4">
                        <TimelineDot
                          tone={status?.attendance?.in_time ? 'emerald' : 'slate'}
                        />
                        <div className="min-w-0 flex-1 border-b border-slate-100 pb-4">
                          <div className="flex items-center justify-between gap-2">
                            <p className="m-0 text-sm font-medium text-slate-800">Punch in</p>
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1 border-b border-slate-100 pb-4">
                              ...
                              <p className="m-0 mt-1 text-sm tabular-nums text-slate-600">
                                ...
                                {isOpenBreak ? (
                                  <span className="font-medium text-amber-700">Now</span>
                                ) : (
                      ...
                      {!breaks.length ? (
                        <li className="relative flex gap-3 pb-4">
                          ...
                          <div className="min-w-0 flex-1 border-b border-slate-100 pb-4">
                            <p className="m-0 text-sm text-slate-400">No breaks</p>
                          </div>
                        </li>
                      ) : null}
                      ...
                      <li className="relative flex gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            <p className="m-0 text-sm font-medium text-slate-800">Punch out</p>
                            <p className="m-0 text-sm font-semibold tabular-nums text-slate-900">
