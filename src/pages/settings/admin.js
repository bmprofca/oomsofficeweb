...
const isAdminAccepted = (row) => Boolean(row?.is_accepted);
...
const ViewAdmins = () => {
    ...
    const loadAdminList = useCallback(async () => {
        ...
        try {
            ...
            setAdminRows(Array.isArray(result.data) ? result.data : []);
            ...
            setAdminRows([]);
            setError(e.response?.data?.message || e.message || 'Failed to load admin list');
        } finally {
    ...
    useEffect(() => {
        const handleClickOutside = (e) => {
            if (
                !e.target.closest('.admin-list-dropdown') &&
                !e.target.closest('[data-admin-list-actions-menu]')
            ) {
    ...
    const activeItem = useMemo(
        () => adminRows.find((row) => row.map_id === activeRowDropdown) || null,
        [adminRows, activeRowDropdown]
    );
    ...
    const handleVerifyEmail = async () => {
        ...
        setCheckFeedback({ type: null, message: '' });
        try {
            const result = await checkAdminUser(email);
            ...
            if (message === 'Admin already exists') {
                setCheckFeedback({
                    type: 'warning',
                    message: 'This admin is already added and has accepted the invitation.',
                });
                return;
            }

            if (message === 'Admin already exists but not accepted yet') {
                setCheckFeedback({
                    type: 'info',
                    message: 'Invitation already sent. Waiting for the admin to accept.',
                });
                return;
            }
            ...
            console.error('Admin check-user:', e);
            setCheckFeedback({
                type: 'error',
    ...
    const handleSendInvitation = async () => {
        ...
        setInviting(true);
        try {
            const result = await createAdminInvitation(checkedUser.username);
            if (result?.success) {
    ...
    const handleToggleStatus = (row) => {
        if (!row?.username || !isAdminAccepted(row)) return;
        ...
        showConfirm({
            ...
            title: `${nextStatus === 'active' ? 'Activate' : 'Deactivate'} Admin`,
            message: `Are you sure you want to ${label} ${row.profile?.name || row.username}?`,
            confirmText: nextStatus === 'active' ? 'Activate' : 'Deactivate',
            cancelText: 'Cancel',
            onConfirm: async () => {
                try {
                    const result = await changeAdminStatus(row.username, nextStatus);
                    if (result?.success) {
                        await loadAdminList();
                        return {
                            variant: 'success',
                            title: 'Status Updated',
                            message: result.message || 'Admin status updated successfully.',
                        };
                    }
                    return {
                        variant: 'error',
                        title: 'Update Failed',
                        message: result?.message || 'Could not update admin status.',
                    };
                } catch (e) {
                    console.error('Admin change-status:', e);
                    return {
                        variant: 'error',
                        title: 'Error',
                        message: e.response?.data?.message || e.message || 'Failed to update admin status.',
                    };
                }
            },
        });
    ...
    const handleResendInvitation = (row) => {
        if (!row?.map_id || isAdminAccepted(row)) return;
        setActiveRowDropdown(null);
        showConfirm({
            variant: 'confirm',
            ...
            onConfirm: async () => {
                try {
                    const result = await resendAdminInvitation(row.map_id);
                    if (result?.success) {
                        const link = result.data?.invitation_link;
                        if (link) {
                            try {
                    ...
                    console.error('Admin resend-invitation:', e);
                    return {
                        variant: 'error',
                        title: 'Error',
    ...
    const handleDelete = (row) => {
        ...
        showConfirm({
            ...
            onConfirm: async () => {
                try {
                    const result = await deleteAdmin(row.map_id);
                    if (result?.success) {
                        await loadAdminList();
                        return {
                            variant: 'success',
                    ...
                    return {
                        variant: 'error',
                        title: 'Delete Failed',
                        message: result?.message || 'Could not delete admin.',
                    };
                } catch (e) {
                    ...
                    return {
                        variant: 'error',
                        title: 'Error',
                        message: e.response?.data?.message || e.message || 'Failed to delete admin.',
                    };
                }
            },
        });
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <div className={`pt-16 transition-all duration-300 ${isMinimized ? 'md:pl-20' : 'md:pl-[260px]'}`}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <div className="mb-6">
                        <h1 className="text-xl font-bold text-slate-900">Branch Admins</h1>
                        <p className="text-sm text-slate-500 mt-1">
                            Invite and manage administrators for this branch
                        </p>
                    </div>
                                    ...
                                    {!loading && rowsForTable.length === 0 ? (
                                        <tr>
                                            <td colSpan="7" className="px-6 py-14 text-center">
                                                ...
                                                <p className="text-sm text-slate-500 mt-1">Add an admin by email invitation</p>
                                            </td>
                                        </tr>
                                    ) : (
                                                ...
                                                <tr key={row.map_id} className="hover:bg-slate-50/80">
                                                    ...
                                                    <td className="px-4 py-4">
                                                        <div className="flex justify-center">
                                                            <button
                                                                type="button"
                                                                className="admin-list-dropdown p-1.5 text-slate-400 hover:text-slate-700 rounded hover:bg-slate-100"
            ...
            {activeRowDropdown && activeItem &&
                createPortal(
                    <motion.div
                        ...
                        data-admin-list-actions-menu
                        className="admin-list-dropdown fixed bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden z-[9999]"
                        style={{
                            top: dropdownPos.top,
                        ...
                        >
                            <div className="shrink-0 flex items-center justify-between gap-2 px-3.5 py-2.5 border-b border-indigo-500/20 bg-indigo-600 text-white">
                                <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <h3 className="text-sm font-semibold truncate">Invite Admin</h3>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => {
                            ...
                            >
                                <p className="text-[11px] leading-relaxed text-slate-500">
                                    Enter the user&apos;s platform email to verify, then send the branch admin invitation.
                                    Only the branch owner can invite new admins.
                                </p>
